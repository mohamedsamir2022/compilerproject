...
char* extractSubstring(char* realStr, int start, int end) {
    int i;

    char* str = (char*)malloc(sizeof(char) * (end - start + 2));

    for (i = start; i <= end; i++) {
        str[i - start] = realStr[i];
        str[end - start + 1] = '\0';
    }
    return str;
}
...
void parseExpression(char* str) {
    ...
    while (right <= len && left <= right) {
        ...
        if (isPunctuator(str[right]) == true && left == right) {
            ...
            char* sub = extractSubstring(str, left, right - 1);

            if (isKeyword(sub) == true) {
                cout << sub << " IS A KEYWORD\n";
            } else if (isNumber(sub) == true) {
                cout << sub << " IS A NUMBER\n";
            } else if (isValidIdentifier(sub) == true
                       && isPunctuator(str[right - 1]) == false) {
                cout << sub << " IS A VALID IDENTIFIER\n";
            } else if (isValidIdentifier(sub) == false
                       && isPunctuator(str[right - 1]) == false) {
...
int main() {
    char inputString[100] = "while(x==y) 22*5+x-9;";
